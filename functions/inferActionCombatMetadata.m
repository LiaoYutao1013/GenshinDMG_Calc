function meta = inferActionCombatMetadata(member, action, archetypeInfo, teamContext)
    ...
    meta = struct( ...
        ...
        'AdditionalTriggeredFollowUpProfiles', repmat(localEmptyTriggeredFollowUpProfile(), 1, 0), ...
    ...
    meta = resolveInferredAuraMetadata(member, action, meta);
    ...
    switch char(normalizedName)
        ...
        case 'fischl'
            if strcmp(string(getFieldOrDefault(meta, 'EffectTag', "")), "Oz")
                ...
                meta = localAppendTriggeredFollowUpProfile(meta, localMakeTriggeredFollowUpProfile( ...
                    "OzA4Retribution", 0.05, 1.0, "Electro", "", 0.0, strings(1, 0), false, inf, ...
                    "ReactionEventTrigger", ...
                    ["ElectroCharged", "Overload", "Superconduct", "StellarConduct", ...
                     "Quicken", "Aggravate", "Hyperbloom", "LunarCharged"], ...
...
function meta = localAppendTriggeredFollowUpProfile(meta, profile)
    profiles = getFieldOrDefault(meta, 'AdditionalTriggeredFollowUpProfiles', repmat(localEmptyTriggeredFollowUpProfile(), 1, 0));
    profiles(end + 1) = profile; %#ok<AGROW>
    meta.AdditionalTriggeredFollowUpProfiles = profiles;
end
...
function profile = localMakeTriggeredFollowUpProfile(action, delay, gauge, element, preferredAura, internalCooldown, eligibleClasses, foregroundOnly, maxTriggerCount, driverMode, reactionNames, packetSources, requireForegroundTrigger)
    ...
    profile.Gauge = double(gauge);
    profile.Element = string(element);
    profile.PreferredAura = string(preferredAura);
    profile.InternalCooldown = double(internalCooldown);
    profile.EligibleClasses = string(eligibleClasses);
    profile.ForegroundOnly = logical(foregroundOnly);
    profile.MaxCount = double(maxTriggerCount);
    profile.DriverMode = string(driverMode);
    profile.AllowedReactionNames = string(reactionNames);
    profile.AllowedPacketSources = string(packetSources);
    profile.RequireForegroundTrigger = logical(requireForegroundTrigger);
end
