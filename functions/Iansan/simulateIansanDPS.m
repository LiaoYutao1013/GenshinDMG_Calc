function [totalDMG, dps, breakdown, rotationTime, audit] = simulateIansanDPS(build, enemy, seqFile, talentLevel, constellation, teamContext)
    ...
    if nargin < 6 || isempty(teamContext)
        teamContext = buildTeamContext({struct('Name', 'Iansan', 'Constellation', constellation, 'Build', build)}, 20, struct(), enemy);
    end
                ...
                if state.BurstTime > 0
                    ...
                    if lunarChargedEnabled
                        reactionDMG = calcReactionDamage(getTalentValue(talent, 'Reaction', 'LunarCharged', talentLevel), ...
                            em, enemy, electroResShred, 1 + getFieldOrDefault(build, 'ReactionDMGBonus', 0) ...
                            + getFieldOrDefault(build, 'LunarChargedBonus', 0) ...
                            + getFieldOrDefault(teamContext, 'LunarChargedBonus', 0), ...
