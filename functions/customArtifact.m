function build = customArtifact(selectedWeapon)
    if nargin < 1, selectedWeapon = '帷间夜曲'; end
    % ====================== 在这里修改配置 ======================
    artifactSet = '穹境示现之夜';       % ← 4件套
    ...
    'SkillDMGBonus',    0.00, ...                         % 战技伤害加成（2件套）
    'BurstDMGBonus',    0.00, ...                         % 爆发伤害加成
    'NormalDMGBonus',   0.00, ...                         % 普攻伤害加成
    'ReactionDMGBonus', 0.25, ...                         % 月曜反应伤害提升（擢升/月兆）
    'EM',               120, ...                          % 元素精通（月感电/绽放关键）
    'ResShred',         0.90, ...                         % 元素抗性削减（队友或套装）
    'Set4_MoonPromote',     0.00, ...                     % 擢升（月曜反应伤害提升）
    'Set4_GravityBonus',    0.20, ...                     % 引力值积攒速度/矩波干涉加成
    'Set4_InterfereBonus',  0.15, ...                     % 矩波干涉额外伤害倍率
    'PromoteBonus',         0.075, ...                    % 通用擢升（天赋/命座）
    'TeamHPAbove50Mult',    0.00 ...                      % 队友>50%HP乘区（芙宁娜类似机制）
    );

    % 保存配置
    writetable(struct2table(build), '../data/artifacts_Columbina.csv');
