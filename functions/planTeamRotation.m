...
function candidate = localEvaluatePlanCandidate( ...
    ...
    [score, summary, metrics] = localScorePlanCandidate(rotationPlan, timelineResult, roles, memberJobs, rotationDuration);
...
function [score, summary, metrics] = localScorePlanCandidate(rotationPlan, timelineResult, roles, memberJobs, rotationDuration)
    ...
    reactionDensity = 0;
    if istable(timelineTable) && height(timelineTable) > 0
        reactionColumn = "";
        if ismember('Reaction', string(timelineTable.Properties.VariableNames))
            reactionColumn = "Reaction";
        elseif ismember('Reactions', string(timelineTable.Properties.VariableNames))
            reactionColumn = "Reactions";
        end
        if strlength(reactionColumn) > 0
            reactionRows = sum(strlength(strtrim(string(timelineTable.(char(reactionColumn))))) > 0);
    ...
    metrics = struct( ...
        ...
        'OccupiedFraction', occupiedFrac, ...
        'ReactionDensity', reactionDensity, ...
        'MissingEnergyRatio', missingRatio, ...
        'IdleFraction', idleFrac, ...
        'OverlapFraction', overlapFrac, ...
        'BurstCoverage', localComputeBurstCoverage(rotationPlan), ...
        'CarryWindowQuality', localComputeCarryWindowQuality(rotationPlan, rotationDuration), ...
        'SetupCoverage', setupCoverage, ...
        'OrderPenalty', orderPenalty);

    summary = sprintf('score=%.3f loop=%.2f react=%.2f idle=%.2fs overlap=%.2fs energy=%.2f', ...
        score, loopReadiness, ...
        reactionDensity, ...
...
function tf = localShouldPreferNamedSupportTemplate(normalizedName)
    tf = any(normalizedName == [ ...
        "furina", "nilou", "nicole", "zhongli", "venti", "kaedeharakazuha", "nahida", ...
