function [totalDMG, dps, breakdown, rotationTime] = simulateWandererDPS(build, enemy, seqFile, talentLevel, constellation, teamContext)
    ...
    actions.E = struct('TalentGroup', "Skill", 'Param', "SkillDMG", 'DamageField', "SkillDMGBonus", ...
        ...
        'PostSetAuxCounter', windFavoredPoints, 'Note', "Windfavored entry");
    % 风姿态下的空居·不伤/空居·刀风倍率记录在 Skill 天赋表中，
    % 这里显式使用 Skill 分组，避免去 Normal 表读取不到对应字段。
