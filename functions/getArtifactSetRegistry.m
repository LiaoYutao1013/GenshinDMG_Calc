...
function entries = localBuildLunarisEntries(catalog)
    ...
    entries = repmat(localEmptyEntry(), 1, numel(artifacts));
    for i = 1:numel(artifacts)
        ...
        entries(i) = localMakeEntry(internalId, displayName, shortLabel, themeColor, apiSlug, iconKey, effect2, effect4, isImplemented);
    end
end
...
function tf = localIsArtifactSetImplemented(setId)
    implemented = string({ ...
        ...
        'ResolutionOfSojourner', 'BraveHeart', 'DefendersWill', 'TinyMiracle', 'Berserker', ...
        'MartialArtist', 'Instructor', 'Gambler', 'TheExile', 'Adventurer', 'LuckyDog', ...
        'Scholar', 'TravelingDoctor', 'BlizzardStrayer', 'Thundersoother', 'Lavawalker', 'MaidenBeloved', ...
        ...
        'HuskOfOpulentDreams', 'OceanHuedClam', 'DeepwoodMemories', 'GildedDreams', ...
        'DesertPavilionChronicle', 'FlowerOfParadiseLost', 'NymphsDream', 'VourukashasGlow', ...
        'MarechausseeHunter', 'GoldenTroupe', 'SongOfDaysPast', ...
    ...
    tf = any(strcmpi(char(string(setId)), cellstr(implemented.')));
end
