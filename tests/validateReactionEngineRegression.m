function validateReactionEngineRegression()
    ...
    ecSwirl = resolveReactionForHit(electroCharged.EnemyState, localMakeHit("Anemo", 1.0), build, teamContext, enemy, 0);
    assert(strcmpi(char(ecSwirl.PrimaryReaction), 'Swirl'), ...
        'Anemo on Electro-Charged should still trigger Swirl.');
    assert(abs(localAuraGauge(ecSwirl.EnemyState, "Hydro") - localAuraGauge(electroCharged.EnemyState, "Hydro")) < 1e-9, ...
        'Single-target Anemo on Electro-Charged should preserve the Hydro side when the Electro side is the first simultaneous-reaction target.');
    assert(localAuraGauge(ecSwirl.EnemyState, "Electro") < localAuraGauge(electroCharged.EnemyState, "Electro"), ...
        'Single-target Anemo on Electro-Charged should consume the Electro side first instead of incorrectly consuming Hydro.');

    ecCrystallize = resolveReactionForHit(electroCharged.EnemyState, localMakeHit("Geo", 1.0), build, teamContext, enemy, 0);
    assert(strcmpi(char(ecCrystallize.PrimaryReaction), 'Crystallize'), ...
        'Geo on Electro-Charged should still trigger Crystallize.');
    assert(abs(localAuraGauge(ecCrystallize.EnemyState, "Hydro") - localAuraGauge(electroCharged.EnemyState, "Hydro")) < 1e-9, ...
        'Single-target Geo on Electro-Charged should preserve the Hydro side when the Electro side is the first simultaneous-reaction target.');
    assert(localAuraGauge(ecCrystallize.EnemyState, "Electro") < localAuraGauge(electroCharged.EnemyState, "Electro"), ...
        'Single-target Geo on Electro-Charged should consume the Electro side first instead of incorrectly consuming Hydro.');
