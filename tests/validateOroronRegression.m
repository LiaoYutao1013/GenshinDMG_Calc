function validateOroronRegression()
    ...
    teamResult = simulateTeamDPS(struct( ...
        'Members', {{diluc, ororonEC, kachina}}, ...
        'RotationDuration', 20, ...
        'SharedBuffs', struct('ReactionMode', "Realistic")), enemy);
    teamOroronRows = teamResult.Breakdown(strcmp(string(teamResult.Breakdown.Character), "Ororon"), :);
    teamPlanNames = string({teamResult.PlannedRotation.MemberPlans.Name});
    teamOroronPlan = teamResult.PlannedRotation.MemberPlans(find(strcmpi(teamPlanNames, "Ororon"), 1, 'first'));
    teamOroronTokens = string(teamOroronPlan.RotationTokens);

    assert(isfinite(teamResult.DPS) && teamResult.DPS > 0, ...
        'Ororon should complete a full team auto-plan simulation without runtime errors.');
    assert(isequal(teamOroronTokens(:)', ["E", "Q"]), ...
        'Ororon team auto-plan should keep the compact public E/Q support sequence.');
    assert(any(strcmp(string(teamOroronRows.Action), "Bounce")) ...
        && any(strcmp(string(teamOroronRows.Action), "Wave")) ...
        && any(strcmp(string(teamOroronRows.Action), "Hypersense")), ...
        'Ororon team auto-plan should still expand into bounce, pulse, and Hypersense damage rows in the direct simulator.');
